        ...
        body = (
            <div className="study-card">
                ...
                <div className={showBack ? 'back' : 'back hide'}>
                    ...
                    <p>How did you do?</p>
                    <p>
                        <button onClick={(event) => onStudied(card.id, Math.max(card.score - 1, 1))}>Poorly</button>
                        <button onClick={(event) => onStudied(card.id, card.score)}>Okay</button>
                        <button onClick={(event) => onStudied(card.id, Math.min(card.score + 1, 3))}>Great</button>
                    </p>
                </div>
            </div>
        );
